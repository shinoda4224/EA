...
input double    entry_jpy                   = 1200000;      // 1エントリーの掛け金(JPY)
input int       ma_length                   = 8;            // 移動平均線の期間(日)
input double    step_pecentage              = 1.9;          // ステップの幅(%)
input double    sl                          = 3.0;          // 損切(%)
input ENUM_TIMEFRAMES     time_frame        = PERIOD_CURRENT;   // 時間足
input bool      Not_allowed_by_hour         = false;        // 時間帯によるエントリー禁止
input bool      Not_allowed_by_day          = false;        // 日によるエントリー禁止
input bool      Not_allowed_by_dayofweek    = false;        // 曜日によるエントリー禁止
input bool      cross_Yen                   = false;        // クロス円=True, ドルストレート=False
...
STEP_VARY Logic() {
    ...
    double      ema             = iMA(NULL, time_frame, ma_length, 0, MODE_EMA, PRICE_CLOSE, 1);
...
bool EA_EntryOrder( 
    ...
    double stop_loss  = 0;          // ストップロス
    
    if ( in_long == true ) {        // Longエントリー
        order_type = OP_BUY;
        order_rate = Ask;
        stop_loss  = NormalizeDouble(Ask * (100 - sl) * 0.01, 3);

    } else {                        // Shortエントリー
        order_type = OP_SELL;
        order_rate = Bid;
        stop_loss  = NormalizeDouble(Bid * (100 + sl) * 0.01, 3);
    }
...
double Lots_calc(){
    ...
    if (money_management == false){
        ...
        order_jpy = 20 * equity * mm_rate * 0.01;
    }
...
bool entry_not_allowed(){
    ...
    if ( Not_allowed_by_day == true ){
        if ( current_day == 25 || current_day == 26 ){
            ret = true;
        }
    }
